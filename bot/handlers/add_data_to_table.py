...
async def add_line_to_table(callback: CallbackQuery, state: FSMContext):
    ...
    message_sent = await callback.message.answer(
        "Отправьте имя клиента", reply_markup=delete_last_keyboard
    )
...
async def get_name(message: Message, state: FSMContext):
    ...
    message_sent = await message.answer(
        "Отправьте цену услуги", reply_markup=delete_last_keyboard
    )
...
async def get_date(message: Message, state: FSMContext):
    ...
    success_message = await message.answer(new_data_added_message)
