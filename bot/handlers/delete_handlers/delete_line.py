...
async def handle_add_line_to_table(callback: CallbackQuery):
    ...
    return await callback.message.answer(
        are_you_sure_to_delete_line_message(table_name=table_name, line=current_client),
          reply_markup= await yes_or_not_delte_line_keyboard(line_id=line_id)
    )
...
async def handle_line_name(callback: CallbackQuery):
    ...
    if not delte_line:
        return await callback.message.answer(line_are_not_deleted_message(table_name=table_name, line=current_client))

    return await callback.message.answer(line_are_deleted_message(line=current_client, table_name=table_name))