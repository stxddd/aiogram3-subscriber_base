...
async def handle_download_table(callback: CallbackQuery):
    ...
    excel_file = await ExcelCRUD.get_excel_file(
        table_id=table_id, table_name=table_name, tg_id=tg_id, clients=clients
    )

    if not excel_file:
        return await callback.message.answer(
            excel_table_can_not_create_error(table_name=table_name)
        )

    file_path = (
        Path(__file__).resolve().parent.parent.parent
        / "files"
        / f"{table_name}_{table_id}_{tg_id}.xlsx"
    )
