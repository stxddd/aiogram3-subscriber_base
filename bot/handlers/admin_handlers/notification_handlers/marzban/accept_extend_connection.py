...
from bot.templates.admin_templates.messages_templates import client_wants_to_extend_message, successful_extension_admin_message, successful_extension_message
...
async def handle_payment_check_completed(callback: CallbackQuery):
    ...
    return await callback.message.bot.send_message(
        ...
        client_wants_to_extend_message(username = client.username, connection = connection, new_date_to = new_date_to, old_price = connection.price, new_price = new_price, key = key),
...
async def handle_accept_extend_connection(callback: CallbackQuery):
    ...
    await callback.message.bot.send_message(
        client.tg_id,
        successful_extension_message(username = client.username, connection=connection, new_date_to=new_date_to, old_date_to=old_date_to, old_price = old_price, new_price = new_price)
    )
    ...
    await callback.message.bot.send_message(
        settings.ADMIN_TG_ID,
        successful_extension_admin_message(connection=connection, username=client.username, new_date_to=new_date_to, old_date_to=old_date_to, old_price = old_price, new_price = new_price)
    )
