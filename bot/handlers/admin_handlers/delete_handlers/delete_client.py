...
from bot.keyboards.admin_keyboards.inline.utils_keyboards import (
    yes_or_not_delte_client_keyboard, cancel_delete_last_keyboard
)
...
async def handle_prepare_to_delete_client(callback: CallbackQuery, state: FSMContext):
    ...
    return await callback.message.answer(
        are_you_sure_to_delete_client_message(
            table_name=table_name, client=current_client, connections=connections
        ),
...
async def handle_delete_client(callback: CallbackQuery, state: FSMContext):
    ...
    return await callback.message.answer(
        enter_code_for_delete_client(client_name=current_client_name),
...
async def handle_delete_secret_key(message: Message, state: FSMContext):
    ...
    if code != settings.KEY_FOR_DELETE:
        ...
        table = await TableDAO.find_one_or_none(id=current_client.table_id)
        if not table:
            await message.answer(table_does_not_exist_error)
        ...
        if not delete_client:
            await message.answer(client_are_not_deleted_message(
                table_name=table_name, client=current_client
            ))

        await message.answer(client_are_deleted_message(
            client=current_client, table_name=table_name, connections=connections
        ))
